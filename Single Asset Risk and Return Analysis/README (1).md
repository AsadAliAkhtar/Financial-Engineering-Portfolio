...
This project conducts a comprehensive risk and return analysis on the **S&P 500 Index (^GSPC)** to understand its historical performance and risk characteristics over a 9-year period. While the S&P 500 is technically an index rather than an individual stock, it serves as an ideal candidate for this analysis because it represents a diversified basket of 500 large-cap U.S. companies, providing more stable and representative results than a single security might offer.
...
- $\sqrt{252}$ = Square root of trading days (volatility scales with square root of time)
...
![S&P 500 Price and Returns](./sp500_price_returns.png)
*Replace with: Chart showing S&P 500 price trend in top panel and daily returns in bottom panel*
...
![30-Day Rolling Volatility](./rolling_volatility.png)
*Replace with: Chart showing S&P 500 returns in top panel and 30-day rolling volatility in bottom panel*
...
│
├── Single_Asset_Risk_Return_Analysis.ipynb  # Main analysis notebook
├── README.md                                 # Project documentation
│
└── visualizations/
    ├── sp500_price_returns.png              # Price and returns chart
    └── rolling_volatility.png               # Rolling volatility chart
```
...
```bash
jupyter notebook Single_Asset_Risk_Return_Analysis.ipynb
```

### Customization

The analysis can be easily adapted to analyze different assets:

```python
# Change the ticker symbol
df = yf.download(tickers="AAPL",  # or any other ticker
                 start='2017-01-01', 
                 end='2025-12-30',
                 auto_adjust=True)
```

**Suggested Assets for Analysis:**
- Individual stocks: AAPL, MSFT, TSLA, GOOGL
- Other indices: ^DJI (Dow Jones), ^IXIC (NASDAQ), ^RUT (Russell 2000)
- International indices: ^FTSE (UK), ^N225 (Japan), ^GDAXI (Germany)
...
- NaN values from first day's return calculation are removed

### Statistical Assumptions
- Returns are assumed to be independent (though autocorrelation may exist)
...
All charts and visualizations created by: **Asad Ali Akhtar**
...
- Analysis inspired by modern portfolio theory and quantitative risk management practices
